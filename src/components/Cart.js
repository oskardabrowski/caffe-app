  ...
  .container {
    ...
    &-steps {
      ...
      & > .stepHalfColor {
        ...
        font-family: "Rubik";
        font-size: 0.9rem;
        background-color: rgba(98, 56, 0, 0.5);
      }
    }
    ...
    try {
      if (action == "increment" && step < 4) {
        if (step == 1 && cartItems.length > 0) {
          ...
          if (
            countrySelection.vlaue !== "" &&
            nameAndSurname.value !== "" &&
            clientEmail.value !== "" &&
            clientCity.value !== "" &&
            clientCityPostcode.value !== "" &&
            clientCityStreet.value !== "" &&
            clientCityHome.value !== ""
          ) {
            ...
            const DeliveryMethodChecked = DeliveryMethod.querySelector(
            ...
            setClientData({
              ...
              clientTelephone: clientTelephone.value,
              clientCity: clientCity.value,
              clientCityPostcode: clientCityPostcode.value,
              clientCityStreet: clientCityStreet.value,
              clientCityHome: clientCityHome.value,
              clientHomeFlat: clientHomeFlat.value,
              clientMessage: clientMessage.value,
              Order: OrderString,
              DeliveryMethod: DeliveryArr[0],
              DeliveryCost: DeliveryArr[1],
              TraditionalPaymentNumber: PayNumber,
              TotalCost: newCost,
            });
            setStep((step += 1));
            PaymentCheckHandler();
      ...
      <div className="container">
        <div className="container-close">
          ...
          <AiOutlineClose
            ...
            className="container-close-ico"
          />
        </div>
        <div className="container-steps">
          <div className={`${step >= 1 ? "fullColor" : "stepHalfColor"}`}>
            Step 1
          </div>
          <div className={`${step >= 2 ? "fullColor" : "stepHalfColor"}`}>
            Step 2
          </div>
          <div className={`${step >= 3 ? "fullColor" : "stepHalfColor"}`}>
            Step 3
          </div>
          <div className={`${step >= 4 ? "fullColor" : "stepHalfColor"}`}>
            Finish
          </div>
        </div>
        <div className="container-windows">
            ...
            {cartItems && cartItems.length > 0 ? (
              cartItems.map((el, index) => {
                try {
                  ...
                  return (
                    <div key={index} className="container-windows-items-item">
                      ...
                      <div className="container-windows-items-item-info">
                        <div>
                          ...
                          <p>
                            Cost per coffe:{" "}
                            <span className={`OneCoffeCost`}>{CostNum}</span> zł
                          </p>
                          <p>
                            Cost:{" "}
                            <span className={`CoffeCost-${id}`}>
                              {totalThisCoffe.toFixed(2)}
                            </span>{" "}
                            zł
                          </p>
                        </div>
                        <button
                          onClick={() =>
                            CoffeCartHandler("REMOVE", id, totalThisCoffe)
                          }
                        >
                      ...
                      <div className="container-windows-items-item-count">
                        <button
                          onClick={() => CoffeCartHandler("ADD", id, CostNum)}
                        >
                          <IoMdArrowDropupCircle />
                        </button>
                        <span className={`CoffeCount-${id}`}>{el.count}</span>
                        <button
                          onClick={() => CoffeCartHandler("DEL", id, CostNum)}
                        >
                          <IoMdArrowDropdownCircle />
                        </button>
                      </div>
            ...
            <div>
              ...
              <form id="DeliveryMethod" action="">
                <input
                  type="radio"
                  name="delivery"
                  value="DPDDeliveryMan 15"
                  checked
                />
                DPD Delivery man 15 zł
                <br />
                <input
                  type="radio"
                  name="delivery"
                  value="InPostDeliveryMan 12"
                />
                InPost Delivery man 12 zł
                <br />
                <input
                  type="radio"
                  name="delivery"
                  value="InPostParcelLocker 5"
                />
                InPost parcel locker 5 zł
                <br />
                <input type="radio" name="delivery" value="PostOffice 8" />
                Post Office 8 zł
              </form>
            </div>
          </div>
          <div className="container-windows-payment">
            <form className="container-windows-payment-form">
              <label htmlFor="PaymentMethodCheck">
                Select Payment Method:{" "}
              </label>
              <select
                id="PaymentMethodCheck"
                className="paymentMethodCheck"
            ...
            <div className="paymentMethod container-windows-payment-method">
              {step === 3 && PaymentVisibleMethod == "Traditional" ? (
                <TraditionalMethod payNum={TraditionalPaymentNumber} />
              ) : (
                ""
              )}
              {step === 3 && PaymentVisibleMethod == "Stripe" ? (
                <Stripe clientData={ClientData} payNum={"Stripe"} />
              ) : (
                ""
              )}
            </div>
        ...
        <div className="container-options">
          <div>
            <span>
              <p>Total Cost:&nbsp;</p>
              <p className="CoffeTotalCost">{Number(totalCost).toFixed(2)}</p>
              &nbsp;zł
            </span>
          </div>
          <div>
            {step > 1 && step < 4 ? (
              <button onClick={() => stepHandler("decrement")}>
                <AiOutlineArrowLeft />
                Prev
              </button>
            ) : (
            ...
            {(step > 2 && PaymentVisibleMethod != "Traditional") ||
            step == 4 ? (
              ""
            ) : (
              <button onClick={() => stepHandler("increment")}>
                <span>Next</span>
                <AiOutlineArrowRight />
              </button>
            )}
